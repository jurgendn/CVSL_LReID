...
class Baseline(LightningModule):
    def __init__(
        ...
        if BASIC_CONFIG.USE_PAIRWISE_LOSS:
            self.pair_loss = self.losses["pair"]

        self.models = build_models(
            self.train_shape, self.class_num, self.num_clothes, out_features
        )
        ...
        if BASIC_CONFIG.USE_CLOTHES_LOSS:
    ...
    def train_dataloader(self):
        if BASIC_CONFIG.SAMPLER:
            train_loader = DataLoader(
                self.train_data,
                batch_size=BASIC_CONFIG.BATCH_SIZE,
                shuffle=False,
                num_workers=BASIC_CONFIG.NUM_WORKER,
                pin_memory=BASIC_CONFIG.PIN_MEMORY,
    ...
    def training_step(self, batch, batch_idx) -> Dict:
        ...
        if BASIC_CONFIG.USE_CLOTHES_LOSS:
            ...
            if self.current_epoch >= BASIC_CONFIG.START_EPOCH_CC:
        ...
        if BASIC_CONFIG.USE_PAIRWISE_LOSS:
            ...
            loss += pair_loss * BASIC_CONFIG.WEIGHT_PAIR

        if BASIC_CONFIG.USE_CLOTHES_LOSS:
            clothes_adv_loss = self.cal(new_pred_clothes, a_cloth_id, pos_mask)
            if self.current_epoch >= BASIC_CONFIG.START_EPOCH_ADV:
                loss += clothes_adv_loss

        if self.use_warm_epoch:
            # Warm Up
            warm_iteration = (
                round(self.dataset_size / BASIC_CONFIG.BATCH_SIZE) * self.warm_epoch
            )  # first 5 epoch
