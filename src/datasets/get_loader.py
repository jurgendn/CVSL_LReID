...
def get_query_gallery_loader():
    query_data = TestDataset(
        json_path=conf.QUERY_JSON_PATH,
        transforms=conf.TEST_TRANSFORM,
        cloth_changing=conf.CLOTH_CHANGING_MODE,
    )
    gallery_data = TestDataset(
        json_path=conf.GALLERY_JSON_PATH,
        transforms=conf.TEST_TRANSFORM,
        cloth_changing=conf.CLOTH_CHANGING_MODE,
    )
...
def get_train_data(orientation_guided):
    if orientation_guided:
        ...
        train_data = TrainDataset(
            conf.TRAIN_PATH, conf.TRAIN_JSON_PATH, conf.TRAIN_TRANSFORM
        )
