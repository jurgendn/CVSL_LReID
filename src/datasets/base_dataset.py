...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 0 <= a_orientation <= 9 or 63 <= a_orientation <= 71 or 27 <= a_orientation <= 45:
            ...
            for item in same_id:
                if 45 <= item["orientation"] < 63 or 9 <= item[
                        "orientation"] < 27:
                    # found positive sample of sideway orientation
                    same_id_diff_ori.append(item)
            for item in diff_id:
                if 0 <= item["orientation"] <= 9 or 63 <= item[
                        "orientation"] <= 71 or 27 <= item["orientation"] <= 45:
        ...
        return (a_img_tensor, p_img_tensor,
                n_img_tensor), (a_pose_tensor, p_pose_tensor,
                                n_pose_tensor), a_id
